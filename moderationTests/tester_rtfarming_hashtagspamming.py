...
# Also checks for blacklisted hash tags and tweets with an excessive number of hashtags.

import os, importlib, time
from mocks import mock_t
import traceback

# ht-getter can be installed using pip.
...
#Method for determining if a tweet is valid and if it should be retweeted/liked.
def isValidTweet(tweet : str) -> bool:
    #Upper limit of hashtags in a tweet.
    maxHashTags = 10
    #Should all be in lowercase.
    blackListedHashTags = set(["#apples"])
    
    hashTags = [hashTag.lower() for hashTag in get_hash_tags(tweet)]

    if len(hashTags) > maxHashTags:
        return False

    for hashTag in hashTags:
        if hashTag in blackListedHashTags:
            return False
...
if __name__ == "__main__":
    ...
    print("\nFirst tweet contains a valid hashtag. Second tweet contains a blacklisted hashtag. Third tweet contains too many hashtags.", flush=True)

    mock_t_list = [mock_t("user", "I study English. #langtwt"),
                    mock_t("user", "I study Korean. #langtwt #apples"),
                    mock_t("User", "#a #b #c #d #e #f #g #h #i #j #k #l #m #n #o #p"),
              ]
    
    main(mock_t_list)

    print("\nTest 3 complete.", flush=True)
